...
### `/robot` → Smart Robot Demo action-aware — local skill engine + schema mood/action/eyes/mouth/hardwareCommand (phiên 40 — 2026-07-08)

**Mục tiêu:** demo web phải giống robot thật hơn, không giống chatbot ngu — vẫn chỉ trong phạm vi `/robot`, không dùng Xiaozhi/Lily/ElevenLabs, không khôi phục `/xiaozhi`/`/robotonline`, không sửa NPM/Cloudflare.

**Thêm mới:**
- `src/lib/robot-ai/types.ts` — `RobotChatResult` (`ok/provider/model?/reply/mood/action/eyes?/mouth?/hardwareCommand?/cached?/error?`), thay cho schema cũ `NormalizedReply` (`face/action` 4 giá trị) chỉ dùng cho path chat chính.
- `src/lib/robot-ai/local-skills.ts` — 15 skill (chào, giới thiệu, chào khách, quay trái/phải, nhìn tôi, cười lên, dừng lại, ngủ đi, thức dậy, demo bán hàng/gia đình/bảo vệ/robot, test mic), normalize tiếng Việt (`NFD` strip + `đ/Đ` riêng), match theo cụm từ trọn vẹn, chạy **trước** OpenAI, không tốn round-trip khi khớp.
- `src/lib/robot-ai/openai-provider.ts` — persona mới đầy đủ hơn, JSON schema `mood/action/eyes/mouth`, `max_completion_tokens: 120` (đã verify đúng tham số qua test thật với `gpt-5.4-nano`, không đoán), timeout 20s, fallback text cố định khi lỗi.

**Sửa:**
- `src/app/api/robot/chat/route.ts` — thêm bước local skill trước `resolveReply`; nhánh `deep:true` (CLI agent) vẫn dùng schema cũ nội bộ nhưng quy đổi best-effort sang schema mới trước khi trả về (`LEGACY_FACE_TO_MOOD`/`LEGACY_ACTION_TO_ROBOT_ACTION` trong route) — không phá tính năng deep mode sẵn có (phiên 16/18). Giữ nguyên toàn bộ logic lưu `ConversationMessage`/session/`ActivityLog`.
- `src/lib/robot/useRobotEyes.ts` + `src/components/robot/RobotFaceKiosk.tsx` — thêm `gazeOverride` (ưu tiên cao nhất, trên cả camera/pointer) để `eyes: left/right/up/down/center` từ response thật sự điều khiển được hướng nhìn (không chỉ là badge hiển thị), tự hết hạn sau 1.5s.
- `src/app/robot/page.tsx` — map `mood→RobotFaceState`, `action→RobotGesture` (chỉ 4/18 action có gesture thật vì `RobotGesture` chỉ có `none|wave|nod`, còn lại hiện qua badge), thêm Demo Control Panel (14 nút), Hardware Command Preview panel, memory `localStorage` (`robot_chuoi_chat_history`, tối đa 20 tin, nút Clear memory) — độc lập với DB session (`loadStatus()` vẫn tải từ Postgres như cũ, không xoá), badge provider/mood/action trong bong bóng chat.

**Đã test thật qua curl (không giả lập):** 12 câu local skill → đúng `provider:"local"` + schema đầy đủ, latency 0-3ms (`mày là ai`, `demo bán hàng`, `quay trái`, `ngủ đi`, `chào khách`, `nhìn tôi`, `cười lên`, `thức dậy`, `dừng lại`, `demo gia đình`, `demo bảo vệ`, `demo robot`, `test mic`). Câu ngoài skill (hỏi thời tiết) → `provider:"openai"`, `model:"gpt-5.4-nano"`, JSON hợp lệ đủ field. `/robot` local + prod `200`. `/xiaozhi`, `/robotonline` vẫn `404` cả local lẫn prod. `tsc --noEmit` sạch cho toàn bộ file đổi (2 lỗi còn lại nằm trong `disabled-routes/`, có từ trước, không liên quan). Không log/leak `OPENAI_API_KEY` trong `brainos.log`.

**Chưa làm:** chưa nối phần cứng thật (`hardwareCommand` mới là preview/log), chưa test UI bằng trình duyệt thật (môi trường VPS không có trình duyệt — chỉ verify qua curl + marker HTML SSR), `eslint`/`next lint` chưa chạy được (repo chưa có `eslint.config.js`).

**Không đụng:** Xiaozhi/Lily/ElevenLabs, `disabled-routes/`, `docs/archived-xiaozhi/`, NPM/Cloudflare, `.env`, Postgres.

---

### Reset `/robot` thành 1 màn demo sạch — bỏ toàn bộ panel rối khỏi UI chính (phiên 41 — 2026-07-08)

**Vấn đề:** sau phiên 38-40, `/robot` tích luỹ quá nhiều panel (Chat, Hands-free Voice, Camera capture, Camera tracking/debug, Voice Assistant/OpenAI Realtime, Hardware Command Preview, Demo Control Panel 14 nút, Điều khiển/Status/battery, Event log) — nhìn như dashboard dev, không giống demo sản phẩm.

**Đã làm — viết lại toàn bộ `src/app/robot/page.tsx` (1884 dòng → ~370 dòng):**
- **Bỏ khỏi UI chính:** Hands-free Voice Mode, Camera capture, Camera tracking/debug cam (`RobotVision`), OpenAI Realtime/`RealtimeMicPanel` (Create session/Connect voice/Disconnect), Hardware Command Preview dạng panel lớn, Event log, khối "Điều khiển" 9 nút DB-backed (`/api/robot/command`), Status panel (battery/mode/last_command), Demo Control Panel 14 nút → còn 6, mọi badge provider/mood/action rối trong bong bóng chat.
- **Layout mới — 1 màn, không cần scroll dài lúc mới mở:** `max-w-[1120px]`, grid 2 cột desktop (trái: mặt robot; phải: 6 nút demo + chat), stack dọc mobile. Header nhỏ (`PageHeader`) với 2 badge pill tĩnh ("online", "local skills + OpenAI optional") thay cho khối "Hardware Ready" 4 badge cũ.
- **Mặt robot:** vẫn `RobotFaceKiosk` + `useRobotEyes` (pointer tracking, không camera), dòng trạng thái tiếng Việt riêng bên dưới mặt (`STATUS_TEXT`: Đang nghỉ/Đang nghe/Đang nghĩ/Đang nói/Đang vui/Đang ngủ/Gặp lỗi nhẹ) thay vì nhãn mono `IDLE/HAPPY/...` cũ.
- **6 nút demo cố định:** Chào khách, Mày là ai, Demo bán hàng, Quay trái, Quay phải, Ngủ đi — gửi thẳng text vào `/api/robot/chat`, local skill trả lời ngay (0 nút nào gọi OpenAI).
- **Chat gọn:** chỉ hiển thị 6 tin gần nhất (`CHAT_DISPLAY_LIMIT`), input + nút Gửi + nút Xoá + mic (chỉ hiện nếu trình duyệt hỗ trợ Web Speech API).
- **Mic tối giản:** 1 lượt nghe (`continuous:false`, `interimResults:false`), final transcript tự gửi luôn vào chat — **bỏ hẳn** hands-free loop, OpenAI STT toggle, session id, latency debug, mic meter, VU meter, push-to-talk (để dành cho bản sau nếu cần).
- **`<details>` "Nâng cao"** (đóng mặc định): provider/mood/action/eyes/mouth/hardwareCommand, raw JSON response cuối, ghi chú "OpenAI chưa cấu hình..." (chỉ hiện khi `provider:"fallback"`, không phải banner đỏ to trên UI chính), dòng "Sau này map hardwareCommand sang ESP32-S3."
- **Reset localStorage:** key mới `robot_chuoi_demo_v3_history` (tối đa 20 tin lưu, hiển thị 6 tin gần nhất). Lúc mount, **chủ động xoá** 3 key cũ (`robot_chuoi_history`, `robot_chuoi_clean_history`, `robot_chuoi_chat_history`) — không đọc lại lịch sử rác/tiếng nước ngoài từ các bản trước. Lần đầu vào bản mới, chat chỉ có đúng 1 câu chào tĩnh (không gọi API): *"Xin chào, mình là Chuối đây."* Nút Xoá reset về lại đúng câu chào này + xoá localStorage.
- **Backend không đổi** — `/api/robot/chat` (phiên 40) đã gọn sẵn đúng yêu cầu (`ok/provider/reply/mood/action/eyes/mouth/hardwareCommand`, local skill trước OpenAI), giữ nguyên toàn bộ logic DB/session, không rewrite.

**Đã test thật (curl, không giả lập):** `/robot` local + prod `200`. `/xiaozhi`, `/robotonline` vẫn `404` local + prod. `mày là ai`/`demo bán hàng`/`quay trái` → đúng `provider:"local"` + schema đầy đủ (không đổi so với phiên 40). Grep HTML SSR xác nhận có đủ marker mới ("Robot Chuối", "Demo nhanh", "Chat với Chuối", "Nâng cao") và **không còn** bất kỳ marker nào của panel đã bỏ ("Hands-free", "Camera", "OpenAI Realtime", "Hardware Command Preview", "Event log", "Xiaozhi", "Lily", "ElevenLabs", "Voice Assistant", "Điều khiển", "Trạng thái"). `tsc --noEmit` sạch cho toàn repo (2 lỗi trong `disabled-routes/`, có từ trước, không liên quan).

**Không xoá file:** `RobotVision.tsx`, `RealtimeMicPanel.tsx`, `src/lib/robot/tracking.ts`, các route `/api/robot/{status,command,event,tts,transcribe,realtime-token}` vẫn còn nguyên trên đĩa (dead code với `/robot` bản mới, giống cách `RobotFace.tsx`/`ExpressiveRobotFace.tsx` được giữ lại từ phiên 38) — không import/dùng nữa trong `page.tsx`, cần lại thì xem git history hoặc port logic từ commit trước bản reset.

**Không đụng:** Xiaozhi/Lily/ElevenLabs, `disabled-routes/`, `docs/archived-xiaozhi/`, NPM/Cloudflare, `.env`, Postgres, backend `/api/robot/chat`.

---

### Khôi phục fullscreen robot + tự động nói, vẫn giữ UI 1 màn sạch (phiên 42 — 2026-07-08)

Bản reset phiên 41 dọn quá tay, làm mất 2 chức năng quan trọng: fullscreen mặt robot và tự động đọc câu trả lời. Thêm lại đúng 2 thứ này vào `src/app/robot/page.tsx`, không khôi phục lại panel rối nào khác (không camera, không OpenAI Realtime, không hands-free loop phức tạp).

**Fullscreen robot:** nút "⛶ Toàn màn hình" ở góc trên-phải của face card (đổi thành "⤢ Thoát toàn màn hình" khi đang fullscreen). `faceCardRef` trỏ vào `<div>` bọc `RobotFaceKiosk` (không phải cả trang) — gọi `requestFullscreen()` trên đúng div này, nên chat/demo buttons (nằm ngoài div) tự động không hiển thị trong chế độ fullscreen (hành vi chuẩn của Fullscreen API, chỉ hiển thị subtree của element được request). Có fallback CSS (`fixed inset-0 z-50`) cho trường hợp trình duyệt từ chối `requestFullscreen()` (vd 1 số ngữ cảnh iOS Safari) — layout vẫn "giả lập" fullscreen được. Lắng nghe `fullscreenchange` để đồng bộ state khi user thoát bằng phím Esc/nút back thay vì bấm nút trong app. Mắt/miệng `RobotFaceKiosk` vẫn animate bình thường khi fullscreen (component không đổi, chỉ đổi `className` kích thước).

**Tự động nói:** toggle "🔊 Tự động nói: Bật/Tắt" trong header card Chat, mặc định **bật**, lưu localStorage key `robot_chuoi_auto_speak`. Khi nhận reply từ `/api/robot/chat` và `autoSpeak=true` (và reply không rỗng): `speechSynthesis.cancel()` → tạo `SpeechSynthesisUtterance` (`lang="vi-VN"`, `rate=1.0`, `pitch=1.0`, `volume=1.0`, chọn voice tiếng Việt nếu máy có, không có thì dùng voice mặc định) → `robotState="speaking"` khi bắt đầu đọc → `onend`/`onerror` đưa `robotState` về mood vừa nhận (thường `idle`/`happy`). Nút "⏹ Dừng nói" cạnh đó: `speechSynthesis.cancel()` + `robotState="idle"` ngay lập tức.

**Bấm nói giữ nguyên tối giản** (không đổi so với phiên 41) — 1 lượt nghe Web Speech API, final transcript tự gửi `/api/robot/chat`, nếu `autoSpeak` bật thì reply tự đọc luôn qua cùng cơ chế trên (không có luồng riêng).

**Đã test thật (curl):** `/robot` local + prod `200`. `mày là ai` → đúng `provider:"local"` + schema đầy đủ (backend không đổi). `/xiaozhi`, `/robotonline` vẫn `404` prod. Grep HTML SSR: có "Toàn màn hình", "Tự động nói", "Dừng nói" cộng với marker cũ của phiên 41 ("Robot Chuối", "Demo nhanh", "Chat với Chuối", "Nâng cao"); **không** có lại bất kỳ marker panel rối nào ("Hands-free", "Camera", "OpenAI Realtime", "Hardware Command Preview", "Event log", "Xiaozhi", "Lily", "ElevenLabs", "Voice Assistant", "Điều khiển", "Trạng thái"). `tsc --noEmit` sạch toàn repo. Không log/leak `OPENAI_API_KEY`.

**Chưa test bằng trình duyệt thật** (môi trường VPS không có trình duyệt/loa) — cần user tự bấm Toàn màn hình/Thoát, bật Tự động nói rồi bấm nút demo để nghe Chuối đọc, bấm Dừng nói giữa chừng, thử Bấm nói + nói thử trên Chrome/Safari thật.

**Không đụng:** Xiaozhi/Lily/ElevenLabs/OpenAI Realtime, camera, `disabled-routes/`, NPM/Cloudflare, `.env`, backend `/api/robot/chat`.

---

### Nâng não Chuối — brain profile + demo scenario có kịch bản + language guard (phiên 43 — 2026-07-08)

**Vấn đề:** local skill (phiên 40-41) chỉ trả 1 câu xác nhận cụt ngủn kiểu debug ("Ok, Chuối quay trái.") — người dùng phản hồi demo "vẫn ngu ngu". Cần Chuối trả lời như 1 màn trình diễn nhỏ có ngữ cảnh, không phải chatbot mẫu.

**Phần 1 — check hiện trạng trước khi sửa:** `mày là ai` → `provider:"local"` (đã có từ trước). Câu ngoài kịch bản (`"nói thử một câu thông minh về robot bán hàng"`) → `provider:"openai"`, `model:"gpt-5.4-nano"`, ~3s — xác nhận OpenAI **đã** được gọi đúng cho câu ngoài local skill từ trước, không phải lỗi thiếu wiring. `OPENAI_API_KEY` có sẵn trong `.env` (chỉ check tồn tại, không in giá trị).

**Thêm mới:**
- `src/lib/robot-ai/chuoi-profile.ts` — `CHUOI_PROFILE` (name/identity/role/speakingStyle/hardware/rules), nguồn sự thật duy nhất cho persona — dùng để build system prompt OpenAI, tránh rải chuỗi cứng nhiều nơi.
- `src/lib/robot-ai/demo-scenarios.ts` — 7 scenario (`greet_customer`, `introduce`, `sales_demo`, `turn_left`, `turn_right`, `sleep`, `wake`), mỗi scenario trả reply dài hơn có ngữ cảnh (vd sales_demo: *"Ở quầy bán hàng, Chuối có thể chào khách, giới thiệu món và nhắc chương trình khuyến mãi. Sau này mình còn có thể báo tồn hàng và gọi chủ quầy."*) + `suggestedNextActions` (2-3 nhãn) + `brainNote:"local scenario"`. `RobotEyes` thêm giá trị `"closed"` (scenario `sleep`) — chỉ mô tả trạng thái cho response/tương lai ESP32-S3, `RobotFaceKiosk` đã tự nhắm mắt khi `mood="sleepy"` nên không cần thêm plumbing hiển thị.
- `src/lib/robot-ai/language-guard.ts` — `checkLanguageGuard()` đo tỷ lệ ký tự ngoài dải Latin cơ bản + Latin Extended Additional (dải chứa toàn bộ chữ Việt có dấu dựng sẵn); >40% ký tự lạ → trả `"Chuối chưa nghe rõ. Bạn nói lại bằng tiếng Việt giúp mình nhé."` (`provider:"local"`, `brainNote:"language guard"`), **không** gọi OpenAI. Chỉ chặn script khác Latin (Bengali/Ả Rập/Cyrillic/CJK...) — câu tiếng Anh thuần vẫn rơi xuống OpenAI bình thường như "câu ngoài kịch bản", không bị chặn nhầm. Đã verify công thức bằng `node` trước khi dùng (không đoán range Unicode).

**Sửa:**
- `src/lib/robot-ai/local-skills.ts` viết lại thành v2 theo đúng thứ tự Phần 5: normalize → language guard → demo scenario → robot command (`nhìn tôi`/`cười lên`/`dừng lại`/`demo gia đình`/`demo bảo vệ`/`demo robot`/`test mic`, còn lại từ skill engine v1) → identity fallback (biến thể hỏi danh tính không khớp keyword scenario, vd "tên là gì") → trả `null` cho OpenAI. Mở rộng biến thể khớp theo yêu cầu: "chuối ơi"/"chào"/"xin chào" đều vào `greet_customer`; "quay trái"/"nhìn trái"/"trái" đều vào `turn_left` (tương tự phải); "ngủ đi"/"nghỉ đi" vào `sleep`; "dậy đi"/"thức dậy" vào `wake`. **Đánh đổi đã biết:** khớp bare "trái"/"phải"/"chào" theo đúng yêu cầu Phần 5 có rủi ro false-positive nhỏ với câu chứa các từ này ở nghĩa khác (vd "cháo" bỏ dấu trùng "chao") — chấp nhận vì đây là demo, không phải sản phẩm ngôn ngữ tự nhiên đầy đủ.
- `src/lib/robot-ai/openai-provider.ts` — system prompt build từ `CHUOI_PROFILE` thay vì chuỗi cứng; thêm `SIMULATOR_CONTEXT` vào user content mỗi request ("Chuối đang là simulator trên web /robot. Sau này hardwareCommand sẽ map sang servo/motor/TFT. Mục tiêu demo là nhìn giống robot thật, không giống chatbot."). Schema JSON output không đổi (`reply/mood/action/eyes/mouth`).
- `src/lib/robot-ai/types.ts` — `RobotChatResult` thêm `suggestedNextActions?: string[]` và `brainNote?: string`.
- `src/app/api/robot/chat/route.ts` — response JSON + metadata lưu DB thêm `suggestedNextActions`/`brainNote` (route logic khác không đổi).
- `src/app/robot/page.tsx` — bấm demo button/gửi chat giờ giữ `robotState="thinking"` tối thiểu 300ms trước khi chuyển speaking (local scenario trả lời <5ms, không có khoảng dừng này thì mặt robot "giật" thinking→speaking gần như tức thời). `suggestedNextActions` hiện thành chip tròn nhỏ dưới tin nhắn cuối, bấm chip tự gửi câu tương ứng (`SUGGESTION_TEXT_MAP`). `<details>` "Nâng cao" thêm dòng `brainNote`/`suggestedNextActions`. Chat bubble vẫn không lộ provider (giữ nguyên từ phiên 41).

**Đã test thật (curl, không giả lập):** `chào khách`/`mày là ai`/`demo bán hàng`/`quay trái` → đúng scenario, câu dài có ngữ cảnh (không còn cụt), `suggestedNextActions` đúng, `brainNote:"local scenario"`, `provider:"local"`, latency 0-3ms (không gọi OpenAI). `ngủ đi`/`nghỉ đi`/`dậy đi`/`quay phải`/`cười lên`/`ten la gi` (biến thể fallback) đều khớp đúng. Câu Bengali (`"চল লফক লফ"`) → `provider:"local"`, `brainNote:"language guard"`, đúng câu xin nói lại tiếng Việt, latency 1ms (không gọi OpenAI). Câu ngoài kịch bản (hỏi thời tiết) → `provider:"openai"`, trả tiếng Việt, không bịa nhận mình là app/chatbot. `/robot` local + prod `200`, `/xiaozhi`/`/robotonline` vẫn `404` prod. `tsc --noEmit` sạch toàn repo (đã fix 1 lỗi `TS2802` do spread string trong `language-guard.ts`, đổi sang `Array.from`). Không log/leak `OPENAI_API_KEY`.

**Chưa test bằng trình duyệt thật** (môi trường VPS không có trình duyệt) — cần user tự bấm chip suggestedNextActions, xem khoảng dừng thinking 300ms có mượt không, nghe Chuối tự đọc câu dài hơn có ổn không.

**Không đụng:** Xiaozhi/Lily/ElevenLabs/OpenAI Realtime, camera, `disabled-routes/`, NPM/Cloudflare, `.env`, UI 1 màn sạch (không thêm panel mới, chỉ thêm chip nhỏ + delay 300ms).
